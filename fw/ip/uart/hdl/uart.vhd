...
ENTITY uart IS
  GENERIC(
    ...
    c_parity_eo :  INTEGER  := 0);        --0 for even, 1 for odd parity
  PORT(
    clk       :  IN   STD_LOGIC;                             --system clock
    reset_n   :  IN   STD_LOGIC;                             --ascynchronous reset
    baud_pres :  IN   STD_LOGIC_VECTOR(15 DOWNTO 0);         --baudrate prescale, pres = clk_freq/baudrate
    tx_ena    :  IN   STD_LOGIC;                             --initiate transmission
    tx_data   :  IN   STD_LOGIC_VECTOR(d_width-1 DOWNTO 0);  --data to transmit
    rx        :  IN   STD_LOGIC;                             --receive pin
    rx_busy   :  OUT  STD_LOGIC;                             --data reception in progress
    rx_error  :  OUT  STD_LOGIC_VECTOR(2 DOWNTO 0);          --start, parity, or stop bit error detected
    rx_data   :  OUT  STD_LOGIC_VECTOR(d_width-1 DOWNTO 0);  --data received
    tx_busy   :  OUT  STD_LOGIC;                             --transmission in progress
    tx        :  OUT  STD_LOGIC;                            --transmit pin
    --debug
    dbg_os_pulse        :  OUT  STD_LOGIC);                            --periodic pulse that occurs at the oversampling rate
END uart;
    
ARCHITECTURE logic OF uart IS
  CONSTANT parity_eo : STD_LOGIC := std_logic(to_unsigned(c_parity_eo, 1)(0));
...
BEGIN
  ...
  PROCESS(reset_n, clk)
    ...
    CONSTANT  half_oversampling     :  INTEGER := (2**os_rate)/2;                      --number of clocks in one pwm period
    VARIABLE rx_count :  INTEGER RANGE 0 TO parity+d_width+2 := 0; --count the bits received
    VARIABLE os_count :  INTEGER RANGE 0 TO oversampling_max := 0;        --count the oversampling rate pulses
  BEGIN
    IF(reset_n = '0') THEN                                 --asynchronous reset asserted
      os_count := 0;                                         --clear oversampling pulse counter
      rx_count := 0;                                         --clear receive bit counter
      rx_busy <= '0';                                        --clear receive busy signal
      rx_error <= (OTHERS => '0');                                       --clear receive errors
      ...
      CASE rx_state IS
        WHEN idle =>                                           --idle state
          ...
          IF(rx = '0') THEN                                      --start bit might be present
            IF(os_count < half_oversampling) THEN                          --oversampling pulse counter is not at start bit center
        ...
        WHEN receive =>                                        --receive state
          IF(os_count < oversampling_max) THEN                          --not center of bit
            os_count := os_count + 1;                              --increment oversampling pulse counter
          ...
          ELSE                                                   --center of stop bit
            ...
            rx_error <= rx_buffer(0) & parity_error & NOT rx;    --output start, parity, and stop bit error flag
